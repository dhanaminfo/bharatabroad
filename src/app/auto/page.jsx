...
export default async function AutoPage() {
  const [posts, rssArticles] = await Promise.all([
    getPublishedPosts("auto"),
    getRssArticles(resolveRssCategory("auto")),
  ]);

  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "30px 20px" }}>
      <h1 style={{ marginBottom: 24 }}>Auto</h1>

      <PostsGrid title="Latest Auto Stories" posts={posts} />

      <NewsSection title="Latest Auto Headlines" articles={rssArticles} category="auto" />
    </div>
  );
}
