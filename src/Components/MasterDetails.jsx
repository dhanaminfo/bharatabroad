...
const MasterDetails = () => {
  ...
  const { id } = useParams();
  const [homeMaster, setHomeMaster] = useState({});

  useEffect(() => {
    const fetchData = async () => {
      try {
        const url = "https://www.bharatabroad.com/api/homemaster";
        const response = await fetch(url);
        if (!response.ok) {
          throw new Error("Failed to fetch homeMaster data");
        }
        const homeMasterData = await response.json();

        // Filter homeMasters to find the one with the matching id
        const foundhomeMaster = homeMasterData.find(
          (homeMaster) => homeMaster.id === parseInt(id)
        );

        if (foundhomeMaster) {
          setHomeMaster(foundhomeMaster);
          console.log("homeMaster found:", foundhomeMaster);
        } else {
          console.log(`No homeMaster found with id: ${id}`);
        }
      } catch (error) {
        console.error("Error fetching homeMaster:", error);
      }
    };
      ...
      <section className="d-md-none">
        <Row className="py-3 g-0">
          <Col lg={9}>
            ...
            <Container>
              <div className="mt-3">
                <h2 className=" fw-bold">{homeMaster.HM_Title}</h2>
                <small className=" fw-bold">
                  <span className="text-primary">{homeMaster.HM_Country} </span>
                </small>
              ...
              <Row>
                <Col xs={12} sm={12} md={12} lg={12}>
                  <div className="mt-3 text-secondary-emphasis">
                    {/* <h4 className="fw-bold text-dark">homeMaster Details</h4> */}
                    <p className="mt-2" style={{ textAlign: "justify" }}>
                      {homeMaster.HM_Lead}
                    </p>
                    <p className="mt-2" style={{ textAlign: "justify" }}>
                      {homeMaster.HM_Description}
                    </p>
                  </div>
                </Col>
              </Row>
              ...
              <Row>
                <Col xs={12} sm={12} md={12} lg={12}>
                  <div className="mt-3">
                    ...
                    <p className="mt-2 fw-200">{homeMaster.HM_Organizers}</p>
                    <hr />
                    <h4 className="fw-bold text-dark">Location</h4>
                    <p className="mt-2 ">
                      {homeMaster.enueName},{homeMaster.city},{homeMaster.state}
                      ,{homeMaster.HM_Country}
                    </p>
                  </div>
                </Col>
              </Row>
        ...
        <Container fluid>
          <Row>
            <Col lg={12}>
              <div className="mt-3">
                <h2 className=" fw-bold">{homeMaster.HM_Title}</h2>
                <p className="mb-1 text-primary">{homeMaster.HM_Country}</p>
